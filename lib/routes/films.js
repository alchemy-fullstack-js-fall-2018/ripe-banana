    ...
    .get('/:id', (req, res) => {
        ...
        Promise.all([
            Film.findById(id)
                ...
                .select({ _id: false, title: true, released: true, studio: true, cast: true }).lean(),
            Review.find().where('film').equals(id).select({ review: true, rating: true, reviewer: true })
                .populate({ path: 'reviewer', select: 'name' }).lean()
        ])
            .then(([film, reviews]) => {
